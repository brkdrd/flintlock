...
func _ready() -> void:
	...
	static_body.set_collision_layer(2)
	static_body.set_collision_mask(4)
	print("✓ StaticBody4D created at position: ", static_body.get_position_4d())
	print("  Collision layer: ", static_body.get_collision_layer(), ", mask: ", static_body.get_collision_mask())

	# Test CharacterBody4D
	print("")
	print("--- Testing CharacterBody4D ---")
	var character := CharacterBody4D.new()
	add_child(character)
	character.set_position_4d(Vector4(15.0, 15.0, 15.0, 15.0))
	character.set_velocity(Vector4(1.0, 0.0, 0.0, 0.0))
	character.set_up_direction(Vector4(0, 0, 0, 1))
	print("✓ CharacterBody4D created at position: ", character.get_position_4d())
	print("  Velocity: ", character.get_velocity())
	print("  Up direction: ", character.get_up_direction())

	# Test Area4D
	print("")
	print("--- Testing Area4D ---")
	var area := Area4D.new()
	add_child(area)
	area.set_position_4d(Vector4(20.0, 20.0, 20.0, 20.0))
	area.set_gravity(9.8)
	area.set_gravity_direction(Vector4(0, 0, 0, -1))
	area.set_monitoring(true)
	print("✓ Area4D created at position: ", area.get_position_4d())
	print("  Gravity: ", area.get_gravity(), ", direction: ", area.get_gravity_direction())
	print("  Monitoring: ", area.is_monitoring())

	print("")
	print("=== Demo complete ===")
	print("All 4D physics classes are working!")
	print("- Node4D base class ✓")
	print("- PhysicsBody4D base class ✓")
	print("- RigidBody4D ✓")
	print("- StaticBody4D ✓")
	print("- CharacterBody4D ✓")
	print("- Area4D ✓")
